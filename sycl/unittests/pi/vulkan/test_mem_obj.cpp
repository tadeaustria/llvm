...
struct VulkanTestMemObj : public ::testing::Test {
...
protected:
  ...
  VulkanTestMemObj() = default;

  ~VulkanTestMemObj() = default;
};

TEST_F(VulkanTestMemObj, piMemBufferCreateSimple) {
...
TEST_F(VulkanTestMemObj, piMemBufferAllocHost) {
  const size_t memSize = 1024u;
  pi_mem memObj;
  ASSERT_EQ((plugin.call_nocheck<detail::PiApiKind::piMemBufferCreate>(
...
TEST_F(VulkanTestMemObj, piMemBufferPinnedMappedRead) {
  ...
  ASSERT_EQ(queue->Context_, context_);
...
TEST_F(VulkanTestMemObj, piMemBufferPinnedMappedWrite) {
  const size_t memSize = sizeof(int);
  const int value = 30;
